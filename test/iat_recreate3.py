...
def modify_pe(src, dst, notp, name, plist):
    ...
    for packer in plist:
        ...
        if not os.path.isfile(os.path.join(src, packer, name)):
            continue
...
notp = os.path.join(homedir, src_dir, 'Notpacked')
...
for name in flist:
    modify_pe(src, dst, notp, name, plist)
