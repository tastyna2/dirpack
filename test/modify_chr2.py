...
def modify_pe(dir, name, packer, np_chr):
    modi_chr = {'mexe': 0, 'mread': 0, 'mwrite': 0}
    chr_dict = dict()
    for p in packer:
        if not os.path.isfile(os.path.join(dir, p, name)) or p == "WinUpack":
            continue
        chr_dict[p] = count_sec(dir, p, name)
        for k in modi_chr:
    ...
    for p in packer:
        ...
        with open(os.path.join(dir, p, name), mode='r+b') as f:
...
for name in flist:
    ...
    modify_pe(dst, name, plist, np_chr)
