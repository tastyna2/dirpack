...
def fill_iat(fo, count_dll, dlls, position, section, api_list, nphint):
    pos = position
    for dll in dlls:
            ...
            for api in api_list[dll['name'].lower()]:
                ...
                if not api in dll['api_name']:
                    ...
                    if isinstance(api, int):
                        ...
                        input_byte = nphint[dll['name'].lower()][api] + api.encode() + b'\x00\x00'
...
def notpacked_api_list(path):
    ...
    with open(path, mode='rb') as f:
        ...
        data = f.read(4)
        impva = int.from_bytes(data, "little")
        offsec = pesig + sectab
        ...
        while num_of_sec > 0:
            ...
            if impva >= secva and impva < secva + secsz:
                impra = impva - secva + secra
            num_of_sec -= 1
        if impra:
            ...
            while True:
                ...
                intva = int.from_bytes(data[:4], "little")
                dll_list.append({'namera': namera, 'thunkra': thunkra, 'thunkva': thunkva, 'intva': intva})
            ...
            for dll in dll_list:
                ...
                if dll['intva'] != 0:
                    intra = calc_rawaddr(dll['intva'], sec_info)
                    f.seek(intra, os.SEEK_SET)
                num_of_api = 0

                # record api virtual address in import address table
                api_vaddr_list = []
                while True:
                    data = f.read(4)
                    if data == b'\x00' * 4:
                ...
                for api in api_vaddr_list:
                    ...
                    if apira is None:
                        ...
                        while f.read(1) != b'\x00':
                            countch += 1
                        f.seek(-countch-1, os.SEEK_CUR)
                        apn = f.read(countch).decode()
                        api_name.append(apn)
                        hint_dict[apn] = hint
                dll['api_name'] = api_name
                dll['hint_dict'] = hint_dict
    return dll_list

def modify_pe(dir, notp, name, plist):
    nonpack_path = os.path.join(notp, name)
    nonpacked = notpacked_api_list(nonpack_path)
    count_dll = dict()
    api_list = dict()
    notp_hint = dict()
    for dll in nonpacked:
        count_dll[dll['name'].lower()] = dll['num_of_api']
        api_list[dll['name'].lower()] = dll['api_name']
        notp_hint[dll['name'].lower()] = dll['hint_dict']

    for packer in plist:
        #if name != 'DTCPing.exe':
        #    continue

        if not os.path.isfile(os.path.join(dir, packer, name)):
            continue

        with open(os.path.join(dir, packer, name), mode='rb') as f, open(os.path.join(dir, packer, name), mode="r+b") as g:
            f.seek(lfanew, os.SEEK_SET)
            data = f.read(4)
            pesig = int.from_bytes(data, "little")
            ...
            if impra:
                ...
                last_addr = fill_iat(g, count_dll, dll_list, new_thunkra, last_sec, api_list, notp_hint)
...
notp = os.path.join(homedir, 'PackDataset', 'Notpacked')

for p in plist:
    if os.path.isdir(os.path.join(dst, p)) and p != "WinUpack":
        pexist.append(p)

plist = pexist
flist = os.listdir(notp)
flist.sort()

for name in flist:
    modify_pe(dst, notp, name, plist)
